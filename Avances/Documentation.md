...
>
> **Bug de raíz más grave, encontrado después (sept 2026):** `np.frombuffer(bloque, dtype=np.int16)` reagrupa los bytes en pares FIJOS (0-1, 2-3, 4-5...) desde el byte 0. Si el transporte pierde/agrega un número **impar** de bytes (perfectamente posible — un solo byte, no una muestra completa), el stream se desalinea por un corrimiento impar que ninguna búsqueda dentro de ese array ya reagrupado puede recuperar, sin importar cuánto busque — el reagrupamiento en pares queda fijo desde su creación. Esto explicaba por qué "No se pudo realinear" se repetía frame tras frame de forma persistente en algunos casos (confirmado decodificando a mano un hex real: los bytes tal cual daban un patrón `[2,3,2,2]` sin sentido, pero desplazados 1 byte daban `[2,3,0,1]`, una rotación perfecta y válida de `[0,1,2,3]`). **Fix:** `_buscar_offset_bytes()` prueba ambas paridades de byte (par e impar) por separado, cada una con la verificación de `N_VERIF_SYNC` grupos.
>
> **Nota de performance:** la búsqueda de alineamiento está vectorizada con `numpy.lib.stride_tricks.sliding_window_view` (una sola pasada sobre el bloque) en vez de un loop de Python probando offset por offset — en Raspberry Pi, ese loop era lo bastante lento como para retrasar la lectura del puerto serie y causar aún más desalineamiento (un ciclo vicioso que no se notaba en PC con más CPU disponible).
